...
namespace SalesWebMvc.Services
{
    public class SalesRecordService
    {
        ...
        public async Task<List<SalesRecord>> FindByDateAsync(DateTime? minDate, DateTime? maxDate)
        {
            ...
            return await result.Include(x => x.Saller).Include(x => x.Saller.Departament).OrderByDescending(x => x.Date).ToListAsync();
        }
        ...
        public async Task<List<IGrouping<Departament,SalesRecord>>> FindByDateGroupingAsync(DateTime? minDate, DateTime? maxDate)
        {
            ...
            return await result.Include(x => x.Saller).Include(x => x.Saller.Departament).OrderByDescending(x => x.Date).GroupBy(x => x.Saller.Departament).ToListAsync();
        }
    }
}
